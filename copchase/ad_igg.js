var adManager = {
	...
	reportScore:function (tempScore)
	{
		console.log(6666666666666666,'reportScore',tempScore)
		if (tempScore <= 0)
		{
			return
		}
		...
		{
			GC.report({
				type: GC.Reports.GAME_SCORE,
				score: tempScore, //游戏结束时得分
			});
		}
	},
	...
	vibrateshort:function ()
	{
		...
		if (curTime - adManager.lastVibrateTime < 200)
		{
			console.log(1111111,"震动间隔 时间不够");
			return;
		}
		adManager.lastVibrateTime=curTime;
		if(window.GC){
			if (window.GC.canIUse('vibrateShort')) {
				window.GC.vibrateShort()
			}
		}
	},
	vibrateLong:function ()
	{
		...
		adManager.lastVibrateTime=curTime;
		if(window.GC){
			if (window.GC.canIUse('vibrateLong')) {
				window.GC.vibrateLong()
			}
		}
	},
	lastVibrateTime:0,
